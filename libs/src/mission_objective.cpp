...
// The strategy to pick up the pylon is to grab the claw and move forward for multiple "pulses", and for the final pulse, start flipping the pylon onto the OTV. Once the pylon is flipped, the claw will open to ensure the pylon is fully secured on the OTV interface plate. Delays in this function are used to wait for certain actions to complete and should be altered based on physical conditions.
void flipSilo() {
    // Pulse one
...
void is_it_magnetic() {
    ...
    // This loop is believed to be the cause of a major bug where the OTV will reset to stage01() and repeat the main loop. We hypothesize that the loop overloads the Arduino CPU causing it to reset, however this may not be the case and it could be some other physical or code issue that has not been observed by the team. Although this is a major bug, usually the OTV does get past this bug at some point and is able to finish the objectives under 5 minutes.
    for (int i = 0; i < 10; i++) {
